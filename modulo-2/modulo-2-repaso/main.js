...
function renderPalettes(){
    ...
    for(const onePalette of palettes) {
        ...
        html += `<li class="palettes js-palettes">`;
        html += `<h1>${onePalette.name}</h1>`;
        html += `<div class="palette_colors">`;

        for(const paletteColors of onePalette.colors) {
            html += `<div class="palette__color" style="background-color: #${paletteColors}"></div>`;
        }
